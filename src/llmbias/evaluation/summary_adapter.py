...
def commands(input_path, output_dir, upstream_root=None, python=None):
    root = Path(upstream_root) if upstream_root else legacy_root() / "experiments/summarization_data/evaluation/summary_bias"
    root = root.resolve()
...
def evaluate(input_path, output_dir, upstream_root=None, python=None):
    ...
    if not (root / "summarybias").is_dir():
        raise FileNotFoundError(f"Restore the pinned summary_bias checkout or set upstream_root: {root}")
    output_dir = Path(output_dir).resolve()
