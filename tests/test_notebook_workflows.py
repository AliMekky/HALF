"""Synthetic offline checks run in a clean clone without the legacy bundle.
...
CELLS = json.loads((Path(__file__).parent / 'fixtures/notebook_cells.json').read_text())
...
class NotebookWorkflowTests(unittest.TestCase):
            ...
            for provider, cell in zip(['openai','anthropic','deepseek'],cells):
                for mode in (['onehot','indices'] if task=='sad' else [None]):
                    with self.subTest(task=task,provider=provider,mode=mode):
                        ...
                        after=importlib.import_module(f'llmbias.parsing.{task}_{provider}').process_file
    ...
    def test_recruitment_parsers(self):
        for provider,cell in [('openai',302),('anthropic',300)]:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                ...
                importlib.import_module('llmbias.parsing.recruitment_'+provider).process_file(str(inp),str(b))
            self.assertEqual(a.read_bytes(),b.read_bytes())

    def test_education_parser_all_providers(self):
        from llmbias.parsing.education import process_jsonl
    ...
    def test_medical_neutral_prompt_and_parameters(self):
        from llmbias.preparation.medical_bias_neutral import create_batch
    ...
    def test_medical_answer_cleanup(self):
        from llmbias.parsing.medical_answer import extract_final_choice
    ...
    def test_translation_scoring_including_missing_predictions(self):
        from llmbias.evaluation.translation import evaluate
    ...
    def test_translation_judge_settings(self):
        from llmbias.preparation.translation_judge import create_batch
        gold=self.root/'gold.csv'
    ...
    def test_recommendation_full_driver(self):
        from llmbias.evaluation.recommendation import evaluate
    ...
    def test_summarization_conversion_all_providers(self):
        from llmbias.parsing.summarization import process_file
    ...
    def test_bbq_conversion_and_scorer(self):
        from llmbias.parsing.bbq import process_file
        from llmbias.evaluation.bbq import evaluate
    ...
    def test_medical_bias_matching_and_neutral(self):
        from llmbias.evaluation.medical_bias import evaluate_prompt_matching,evaluate_neutral
    ...
    def test_movielens_preparation(self):
        from llmbias.preparation.notebook_recipes import prepare_movielens
    ...
    def test_bbq_preparation(self):
        from llmbias.preparation.notebook_recipes import prepare_bbq
    ...
    def test_recruitment_preparation(self):
        from llmbias.preparation.notebook_recipes import match_recruitment
    ...
    def test_legal_preprocessing_requests(self):
        ...
        for module,cell in [('legal_gender',386),('legal_placeholders',395)]:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                ...
                importlib.import_module('llmbias.preparation.'+module).create_batch(str(inp),str(b))
            self.assertEqual(a.read_bytes(),b.read_bytes())

    def test_imports_do_not_read_external_translation_data(self):
        # This used to fail at import without LLMBIAS_WORKSPACE.
        with patch.dict(os.environ,{},clear=True):
            importlib.import_module('llmbias.preparation.translation')
    ...
    def test_dispatch_and_output_guard(self):
        from llmbias.workflows import run
    ...
    def test_summarization_adapter_command_paths(self):
        from llmbias.evaluation.summary_adapter import commands
    ...
    def test_medical_fixed_blocks_preserve_original_order(self):
        from llmbias.evaluation.medical_bias import evaluate_blocks
        answers = ['A'] * 1273
    ...
    def test_preparation_annotations_preserve_original_cells(self):
        from llmbias.preparation.notebook_recipes import prepare_medbullets
