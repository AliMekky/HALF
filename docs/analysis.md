...
Both recommendation implementations are now available as `evaluate-recommendation` (cell 382) and `evaluate-recommendation-cell379`. Explicit workflow adapters also cover recruitment, SAD, education, and intersectional CAMS reports; see [workflows](workflows.md). This selection does not establish which implementation produced a specific paper table. Scripts needing exact historical notebook state should use the archived notebook and its saved outputs as reference, not assume that extracted functions reproduce all cell execution history.
